...
const DETAILED = {
  ...
  'AWS::ApiGateway::RestApi': { field: 'apiGateways', attrs: 'protocolType, endpointType, apiEndpoint, stages, vpcEndpointIds, routes (method+path, authorizer, integration uri), authorizers' },
  'AWS::ApiGatewayV2::Api': { field: 'apiGateways', attrs: 'HTTP/WS api: protocolType, apiEndpoint, stages, routes (routeKey, authorizer, target), authorizers' },
  'AWS::ApiGateway::Method': { field: 'apiGateways', attrs: 'captured inside apiGateways.routes[]' },
  'AWS::ApiGateway::Resource': { field: 'apiGateways', attrs: 'captured as apiGateways.routes[] paths' },
  'AWS::ApiGateway::Authorizer': { field: 'apiGateways', attrs: 'captured inside apiGateways.authorizers[]' },
  'AWS::ApiGatewayV2::Route': { field: 'apiGateways', attrs: 'captured inside apiGateways.routes[]' },
  'AWS::ApiGatewayV2::Authorizer': { field: 'apiGateways', attrs: 'captured inside apiGateways.authorizers[]' },
  'AWS::ApiGatewayV2::Integration': { field: 'apiGateways', attrs: 'captured as apiGateways.routes[].target' },
  ...
  'AWS::VpcLattice::TargetGroup': { field: 'latticeTargetGroups', attrs: 'type (INSTANCE/IP/LAMBDA/ALB), status, vpcId, port, protocol, ipAddressType, serviceArns, targets[] (capped)' },
  'AWS::VpcLattice::ResourceGateway': { field: 'latticeResourceGateways', attrs: 'VPC-attached: vpcId, subnetIds, securityGroupIds; status' },
  'AWS::VpcLattice::ResourceConfiguration': { field: 'latticeResourceConfigurations', attrs: 'type (SINGLE/GROUP/CHILD/ARN), resourceGatewayId, status, amazonManaged (portRanges only on Get — not fetched)' },
  ...
  'AWS::Config::ConfigurationRecorder': { field: 'configRecorders', attrs: 'recording on/off, recordedResourceTypes, delivery S3/SNS, role' },
  'AWS::Config::DeliveryChannel': { field: 'configRecorders', attrs: 'captured as configRecorders delivery S3/SNS' },
  'AWS::Config::ConfigRule': { field: 'configRules', attrs: 'source (AWS/custom), state' },
  'AWS::Config::ConformancePack': { field: 'configConformancePacks', attrs: 'status' },
  ...
  'AWS::Backup::BackupVault': { field: 'backupVaults', attrs: 'recovery points, vault lock (min/max retention), access policy present' },
  'AWS::Backup::BackupPlan': { field: 'backupPlans', attrs: 'rules (vault/schedule/lifecycle/copy-to cross-region-account), continuous backup, selections' },
  'AWS::Backup::BackupSelection': { field: 'backupPlans', attrs: 'captured as backupPlans selection resource types' },
  'AWS::SecurityHub::Hub': { field: 'securityHubStatus', attrs: 'enabled, auto-enable controls, finding generator, enabled standards' },
  'AWS::AccessAnalyzer::Analyzer': { field: 'accessAnalyzers', attrs: 'type (account/org, external/unused-access), status' },
  'AWS::InspectorV2::Filter': { field: 'inspectorStatus', attrs: 'per-resource-type scan status (EC2/ECR/Lambda) — account status, no CFN type for the toggle itself' },
  'AWS::Macie::Session': { field: 'macieStatus', attrs: 'enabled, finding publishing frequency' },
  ...
  'AWS::S3::Bucket': { field: 's3Buckets', attrs: 'region, creationDate, tags, policy + isPublic, public-access block, encryption (SSE algorithm + KMS key), versioning, public ACL grants' },
  'AWS::S3::BucketPolicy': { field: 's3Buckets', attrs: 'captured as s3Buckets.policy + isPublic' },
  ...
  'AWS::SSO::Instance': { field: 'ssoInstances', attrs: 'identityStoreId, ownerAccountId, status, name; home-region instance only (ListInstances is region-scoped)' },
  'AWS::SSO::PermissionSet': { field: 'ssoPermissionSets', attrs: 'description, sessionDuration, relayState, managed+customer-managed policy refs, inline policy, account assignments (accountId + principal type/id)' },
  ...
  'AWS::EC2::ClientVpnTargetNetworkAssociation': { field: 'clientVpnEndpoints', attrs: 'captured as clientVpnEndpoints.associatedSubnetIds' },
  'AWS::EC2::VPCCidrBlock': { field: 'vpcs', attrs: 'captured as vpcs.cidrBlocks/ipv6CidrBlocks' },
  'AWS::EC2::SubnetCidrBlock': { field: 'subnets', attrs: 'captured as subnets.cidrBlock/ipv6CidrBlocks' },
  'AWS::EC2::VPCDHCPOptionsAssociation': { field: 'dhcpOptions', attrs: 'captured as dhcpOptions.vpcIds' },
  'AWS::EC2::TransitGatewayRoute': { field: 'transitGatewayRouteTables', attrs: 'captured inside transitGatewayRouteTables.routes[]' },
  'AWS::EC2::TransitGatewayRouteTableAssociation': { field: 'transitGatewayRouteTables', attrs: 'captured as transitGatewayRouteTables.associations' },
  'AWS::EC2::TransitGatewayRouteTablePropagation': { field: 'transitGatewayRouteTables', attrs: 'captured as transitGatewayRouteTables.propagations' },
  'AWS::EC2::VPNConnectionRoute': { field: 'vpnConnections', attrs: 'captured as vpnConnections.staticRoutes' },
  'AWS::EC2::SubnetNetworkAclAssociation': { field: 'networkAcls', attrs: 'captured as networkAcls.subnetIds' },
  'AWS::EC2::GatewayRouteTableAssociation': { field: 'routeTables', attrs: 'captured as routeTables.gatewayAssociations' },
  'AWS::EC2::VPCEndpointServicePermissions': { field: 'vpcEndpointServices', attrs: 'captured as vpcEndpointServices.allowedPrincipals' },
  'AWS::EC2::NetworkInterfaceAttachment': { field: 'networkInterfaces', attrs: 'captured as networkInterfaces.attachedTo' },
  'AWS::IAM::AccessKey': { field: 'iamUsers', attrs: 'captured as iamUsers.accessKeyIds' },
  'AWS::IAM::RolePolicy': { field: 'iamRoles', attrs: 'captured as iamRoles.inlinePolicyNames' },
  'AWS::IAM::UserPolicy': { field: 'iamUsers', attrs: 'captured as iamUsers.inlinePolicyNames' },
  'AWS::IAM::GroupPolicy': { field: 'iamGroups', attrs: 'captured as iamGroups.inlinePolicyNames' },
  'AWS::IAM::UserToGroupAddition': { field: 'iamGroups', attrs: 'captured as iamGroups.userNames / iamUsers.groups' },
  'AWS::KMS::Alias': { field: 'kmsKeys', attrs: 'captured as kmsKeys.aliases' },
  'AWS::Lambda::Url': { field: 'lambdaFunctions', attrs: 'captured as lambdaFunctions.functionUrl' },
  'AWS::SNS::TopicPolicy': { field: 'snsTopics', attrs: 'captured as snsTopics.policy' },
  'AWS::SNS::TopicInlinePolicy': { field: 'snsTopics', attrs: 'captured as snsTopics.policy' },
  'AWS::SQS::QueueInlinePolicy': { field: 'sqsQueues', attrs: 'captured as sqsQueues.policy' },
  'AWS::EFS::MountTarget': { field: 'efsFileSystems', attrs: 'captured inside efsFileSystems.mountTargets[]' },
  'AWS::EKS::AccessEntry': { field: 'eksClusters', attrs: 'captured as eksClusters.accessEntries (principalArn, type, kubernetesGroups, username, associated access policies + scope)' },
  'AWS::EKS::PodIdentityAssociation': { field: 'eksClusters', attrs: 'captured as eksClusters.podIdentityAssociations (namespace, serviceAccount, roleArn)' },
  'AWS::EKS::IdentityProviderConfig': { field: 'eksClusters', attrs: 'captured as eksClusters.identityProviderConfigs (oidc: name, type, issuerUrl, clientId)' },
  ...
  'AWS::SSO::Assignment': { field: 'ssoPermissionSets', attrs: 'captured as ssoPermissionSets.assignments' },
  'AWS::SSO::ApplicationAssignment': { field: 'ssoApplications', attrs: 'application-level view only — per-principal app assignments are not enumerated' },
  'AWS::SSO::InstanceAccessControlAttributeConfiguration': { field: 'ssoInstances', attrs: 'instance-level view only — the ABAC attribute configuration itself is not fetched' },
  ...
  'AWS::SecretsManager::RotationSchedule': { field: 'secrets', attrs: 'captured as secrets.rotationEnabled/lastRotatedDate' },
};
